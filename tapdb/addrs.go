...
type (
	...
	// for an internal key.
	KeyLocator = sqlc.FetchInternalKeyRow
)
...
type AddrBook interface {
	...
	// FetchInternalKey fetches the key locator for an internal key.
	FetchInternalKey(ctx context.Context, rawKey []byte) (KeyLocator,
		error)
}
...
// FetchInternalKey attempts to fetch the key locator information for the given
// raw internal key. If the key cannot be found, then ErrInternalKeyNotFound is
// returned.
func (t *TapAddressBook) FetchInternalKey(ctx context.Context,
