...
// TLV types for tapscript tree encode/decode.
const (
...
// The following taproot tree {en,de}coders are a duplicate of those used in
// btcwallet: https://github.com/btcsuite/btcwallet/blob/master/waddrmgr/tlv.go
// This duplication is needed until we update btcwallet to export these methods.

// EncodeTaprootScript encodes the given internal key and full set of taproot
// script leaves into a byte slice encoded as a TLV stream.
func EncodeTaprootScript(s *waddrmgr.Tapscript) ([]byte, error) {
	if s == nil {
	...
	if len(s.Leaves) > 0 {
		recordSize := func() uint64 {
			...
			err := leavesEncoder(&buf, &s.Leaves, &[8]byte{})
			if err != nil {
				panic(err)
			}
		...
		tlvRecords = append(tlvRecords, tlv.MakeDynamicRecord(
			typeTapscriptLeaves, &s.Leaves, recordSize,
...
// DecodeTaprootScript decodes the given byte slice as a TLV stream and
// attempts to parse the taproot internal key and full set of leaves from it.
func DecodeTaprootScript(tlvData []byte) (*waddrmgr.Tapscript, error) {

	var (
		typ                uint8
		controlBlockBytes  []byte
		fullOutputKeyBytes []byte
		s                  = &waddrmgr.Tapscript{}
	)
	...
	s.Type = waddrmgr.TapscriptType(typ)
